...
#' @param cancer Name of the TCGA project code such as 'BRCA' that is analyzed for miRNA-mRNA correlation. Possible cancer types ACC, BLCA, BRCA, CESC, CHOL, COAD, COADREAD, DLBC, ESCA, GBMLGG, HNSC, KICH, KIPAN, KIRC, KIRP, LGG, LIHC, LUAD, LUSC, OV, PAAD, PCPG, PRAD, READ, SARC, SKCM, STAD, STES, TGCT, THCA, THYM, UCEC, UCS, UVM
...
#' \dontrun{
#' brcaCorr<- corrbased(mirnagene = NoRCE::brain_mirna, cancer = 'BRCA', minAbsCor = 0.3, database)
#'}
#'
...
corrbased <- function(mirnagene,
  ...
  dat <- conn %>%
    dplyr::tbl('cor_mir') %>% dplyr::select(mirna_base, feature, cancer) %>% dplyr::filter(mirna_base %in% mirnagene$g) %>% collect() %>%
    tidyr::gather(cancer, cor,-mirna_base,-feature) %>% mutate(cor = cor /
                                                                 100) %>% dplyr::filter(abs(cor) > minAbsCor) %>%
...
corrbasedMrna <-
  function(mRNAgene, cancer, minAbsCor, databaseFile) {
    ...
    dat <- conn %>%
      dplyr::tbl('cor_mir') %>% dplyr::select(mirna_base, feature, cancer) %>% dplyr::filter(feature %in% mRNAgene$g) %>% collect() %>%
      tidyr::gather(cancer, cor,-mirna_base,-feature) %>% mutate(cor = cor /
                                                                   100) %>% dplyr::filter(abs(cor) > minAbsCor) %>%
      arrange(dplyr::desc(abs(cor))) %>% na.omit()
...
#' @param cancer Name of the TCGA project code such as 'BRCA' that is analyzed for miRNA-mRNA correlation
#' @param databaseFile Path of miRcancer.db file
#'
#' @return Data frame of the raw read count of the given miRNA genes for different patients
#'
#' @examples
#'
#' database<- "filePath//miRCancer.db"
#' \dontrun{
#' brcaCorr<- getmiRNACount(mirnagene = brain_mirna, cancer = 'BRCA', databaseFile = database)
#' }
#'
#' @importFrom DBI dbConnect
#' @importFrom RSQLite SQLite
...
getmiRNACount <- function(mirnagene, cancer, databaseFile) {
  ...
  dat <-
    conn %>% dplyr::tbl('profiles') %>% dplyr::select(study, mirna_base, count) %>% dplyr::filter(mirna_base %in% mirnagene$g) %>%
    dplyr::filter(study %in% cancer) %>% collect() %>% na.omit()

  return(dat)
}

#' Calculates the correlation coefficient values between two custom expression data.
#' @param exp1 Custom expression data matrix. Columns must be genes and rows must be patients.
#' @param exp2 Custom expression data matrix. Columns must be genes and rows must be patients.
...
#' @param corCutoff Correlation cut off values for the given correlation method
...
#'
#' @return Pairwise relations between gene-gene with corresponding correlation value and pvalue
#'
...
calculateCorr <-
  function(exp1,
    ...
    for (i in 1:dim(exp2)[2]) {
      ...
      if(!isEmpty(index)){
        extractData <-
          rbind(extractData, data.frame(label1[index,], label2[i,], corpValue[index, ]))
      }
    }
